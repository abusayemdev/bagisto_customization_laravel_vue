...
use Webkul\Checkout\Models\PickupLocation;
...
class OnepageController extends Controller
{
    ...
    */
    public function summary(Request $request)
    {
        ...
        $zone_id = core()->getCourierZoneId($country_code,$state_code,$city_id);

        $cart = Cart::getCart();

        return response()->json([
            'html' => view('shop::checkout.total.summary', compact('cart','zone_id','payment_method'))->render(),
        ]);
    }
    ...
    */
    public function saveAddress(CustomerAddressForm $request)
    {
        ...
        if (isset($data['shipping']['address_id'])) {
            $shipping_address['country'] =  $data['shipping']['country'] ?? $data['billing']['country'];
            $shipping_address['state'] =  $data['shipping']['state'] ?? $data['billing']['state'];
            $shipping_address['city'] =  $data['shipping']['city'] ?? $data['billing']['city'];
        }else {
            $shipping_address['country'] =  $data['billing']['country'];
            $shipping_address['state'] =  $data['billing']['state'];
            $shipping_address['city'] =  $data['billing']['city'];
        }
        ...
        if (Cart::hasError() || ! Cart::saveCustomerAddress($data)) {
            ...
            if ($cart->haveStockableItems()) {
                if (! $rates = Shipping::collectRates()) {
                    ...
                    return response()->json(['shipping'=>$rates, 'shipping_address' => $shipping_address]);
                }
            } else {
    ...
    */
    public function saveShipping()
    {
        ...
        $courier_zone_id = core()->getCourierZoneId($country_code,$state_code,$city_id);

        $pickup_locations = PickupLocation::where('courier_zone_id', $courier_zone_id)->get();
        ...
        return response()->json([
            ...
            'payment_methods' => Payment::getSupportedPaymentMethods($pickup_locations, $shippingMethod),
            'pickup_locations' => $pickup_locations
        ]);
    }


    public function savePickupLocation()
    {
        $pickup_location_id = request()->get('pickup_location');
       
        Cart::savePickupLocation($pickup_location_id);

        $pickup_location = PickupLocation::find($pickup_location_id);

        Cart::collectTotals();

        return response()->json(['pickup_location' => $pickup_location]);
