...
class Cart
{
    ...
    public function toArray()
    {
        ...
        $base_service_fee =  session()->get('base_service_fee');

        if ($base_service_fee) {
            $data['base_service_fee'] = $base_service_fee;
            $data['service_fee'] = core()->convertPrice($base_service_fee);
        }

        if ($cart->shipping_method == "free_free") {
            $base_shipping_charge = $cart->pickup_location->price;
            $pickup_location_id = $cart->pickup_location_id;
        }else {
            $base_shipping_charge =  session()->get('base_shipping_charge');
        }
        
        if ($base_shipping_charge) {
            $data['base_shipping_charge'] = $base_shipping_charge;
            $data['shipping_charge'] = core()->convertPrice($base_shipping_charge);
        }
    ...
    public function savePickupLocation($pickup_location_id): bool
    {
        if (! $cart = $this->getCart()) {
            return false;
        }

        $cart->pickup_location_id = $pickup_location_id;
        $cart->save();

        return true;
    }

    public function removePickupLocation()
    {
        $cart = $this->getCart();
        $cart->pickup_location_id = null;
        $cart->save();
    ...
    public function collectTotals(): void
    {
        ...
        if ($cart->shipping_method == "free_free" && $cart->pickup_location_id) {
            $base_shipping_charge =  $cart->pickup_location->price;
        }else {
            $base_shipping_charge =  session()->get('base_shipping_charge');
        }
        ...
        $cart->grand_total = $cart->sub_total + $cart->tax_total - $cart->discount_amount + core()->convertPrice($base_shipping_charge) + core()->convertPrice($base_service_fee);
        $cart->base_grand_total = $cart->base_sub_total + $cart->base_tax_total - $cart->base_discount_amount + $base_shipping_charge + $base_service_fee;
