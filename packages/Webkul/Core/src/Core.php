...
use Webkul\Checkout\Models\ServiceCharge;
...
class Core
{
    ...
    public function groupedCitiesByStates()
    {
        ...
        foreach ($this->cityRepository->all() as $city) {
            $collection[$city->state_code][] = $city->toArray();
        }
    ...
    public function getCourierZoneId($country_code, $state_code, $city_id)
    {
        if ($city_id) {
            $city = City::find($city_id);
            
            if ($city->courier_zone_id) {
                $zone_id = $city->courier_zone_id;
            }else {
                
                if ($country_code) {
                    $country_id = Country::where('code', $country_code)->first()->id;
                    if ($state_code) {
                        $state = CountryState::where('country_id', $country_id)->where('code', $state_code)->first();
                        
                        if ($state->courier_zone_id) {
                            $zone_id = $state->courier_zone_id;
                        }else {
                            $zone_id = null;
                        }
    ...
    public function getPaystackCharge($total)
    {
        $paystack_charge_data = ServiceCharge::where('payment_service_name', "paystack")->latest()->first();

        if ($paystack_charge_data) {
            if ($total < $paystack_charge_data['charge_limit']) {
                $paystack_charge = ($paystack_charge_data['percent_of_total']* $total/100);
            }elseif ($total >= $paystack_charge_data['charge_limit']) {
                $paystack_charge = ($paystack_charge_data['percent_of_total']* $total/100) + $paystack_charge_data['additional_charge'];
            }
        } else {
            $paystack_charge = 0;
        }
    ...
    public function getShippingCharge($zone_id, $cart)
    {
        $zone = CourierZone::find($zone_id);
        $shipping_charge_type = $zone->shipping_charge_type;

        if ($shipping_charge_type == "per_total_weight") {

            $weight = $cart->items->sum('base_total_weight');
            $weight_increase_rate = $zone->weight_increase_rate;
            $initial_shipping_charge = $zone->initial_shipping_charge;
            $shipping_charge_increase_rate = $zone->shipping_charge_increase_rate;
        
            for ($i = 0; $i < 100; $i += $weight_increase_rate) {
                if ($weight >= $i && $weight < $i + $weight_increase_rate) {
                    $shipping_weight = $i + $weight_increase_rate;
                    $shipping_charge = (($shipping_weight * $shipping_charge_increase_rate) / $weight_increase_rate) + $initial_shipping_charge - $shipping_charge_increase_rate;
                    break;
                }
            }
            
        }elseif ($shipping_charge_type == "per_product_weight") {
            ...
            foreach ($cart->items as $item) {
                ...
                for ($i = 0; $i < 100; $i += $weight_increase_rate) {
                    if ($weight >= $i && $weight < $i + $weight_increase_rate) {
                        $shipping_weight = $i + $weight_increase_rate;
                        $shipping_charge_per_product = (($shipping_weight * $shipping_charge_increase_rate) / $weight_increase_rate) + $initial_shipping_charge - $shipping_charge_increase_rate;
                        break;
                    }
                }

                $shipping_charge += $shipping_charge_per_product; 
            }
        }
