...
<html lang="{{ config('app.locale') }}">
    ...
    <body @if (core()->getCurrentLocale() && core()->getCurrentLocale()->direction == 'rtl') class="rtl" @endif style="scroll-behavior: smooth;">
        ...
        <!-- select states of a corresponding country -->
        <script type="text/javascript">
            $(document).ready(function() {
                $('select[name="country_name"]').on('change', function(){
                    ...
                    if(country_name) {
                        $.ajax({
                            url: "{{  url('/admin/country-states') }}/"+country_name,
                            type:"GET",
                            dataType:"json",
                            success:function(data) {
                                $('select[name="state_name"]').empty();
                                $.each(data, function(key, value){
                                    $('select[name="state_name"]').append('<option value="'+ value.default_name +'">' + value.default_name + '</option>');
                                });
                            },
                        });
                    } 
